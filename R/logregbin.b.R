    ...
    private = list(
        ...
        .initClassTable = function() {
            ...
            for (i in seq_along(self$options$blocks)) {
                ...
                table$setNote("thres", jmvcore::format(.("The cut-off value is set to {cutoff}"), cutoff=cutOff))
        ...
        .errorCheck = function() {
            ...
            if (length(levels(column)) > 2)
                jmvcore::reject(
                    jmvcore::format(
                        .('The dependent variable "{dep}" has more than two levels; binomial logistic regression can only be performed on dependent variables with two levels.'),
                        dep=dep
                    ),
                    code=''
                )
