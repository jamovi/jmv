...
expect_identical(ttest$results$get("descriptives")$asString(),
    paste(sep="\n",
        "",  
        " Descriptives                                 ",
        " ──────────────────────────────────────────── ",
...
expect_error(TTestPS(data, c("x","y"), hypothesis="error"), "Argument 'hypothesis' must be one of 'different', 'oneGreater', 'twoGreater'", fixed=TRUE)
...
expect_equal(ttest$results$get("normality")$asString(),
    paste(sep="\n",
        "",  
        " Test of Normality (Shapiro-Wilk) ",
        " ──────────────────────────────── ",
...
expect_error(TTestPS(data, c("y","w","z")), "At least one variable not paired", fixed=TRUE)
expect_error(TTestPS(data.frame(x=rep(1,10),y=c(rep(1,9),1.000000000000000001)),c("y","x"), norm=TRUE), "'y - x' is essentially constant", fixed=TRUE)

expect_error(TTestPS(data.frame(x=rep(NA,10),y=c(runif(10))), c("y", "x")), "At least one variable only contains missing values", fixed=TRUE)
expect_error(TTestPS(data.frame(x=runif(10),y=c(Inf,runif(9))), c("y","x")), "Argument 'vars' specifies column 'y' which contains (and must not) infinite values", fixed=TRUE)
