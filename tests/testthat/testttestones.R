...
expect_error(TTestOneS(data, c("x","y"), hypothesis="error"), "Error : Argument 'hypothesis' must be one of 'different', 'oneGreater', 'twoGreater'\n", fixed=TRUE)
...
expect_error(TTestOneS(data.frame(x=rep(1,10),y=c(rep(1,9),1.000000000000000001)),c("y","x"), norm=TRUE), "Error : Variable 'y' has essentially constant values\n", fixed=TRUE)

expect_error(TTestOneS(data.frame(x=rep(NA,10),y=c(runif(10))), c("y", "x")), "Error : Variable 'x' only contains missing values\n", fixed=TRUE)
expect_error(TTestOneS(data.frame(x=runif(10),y=c(Inf,runif(9))), c("y","x")), "Error : Argument 'vars' specifies column 'y' which contains (and must not) infinite values\n", fixed=TRUE)
